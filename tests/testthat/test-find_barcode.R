test_that("barcode_output_file_identical", {
  ...
  expect_identical(
    read.delim("https://raw.githubusercontent.com/mritchielab/FLAMES/devel/tests/testthat/bc_stat"),
    read.delim(file.path(outdir, "stats.tsv"))
  )

  readfq <- function(file) {
    strm <- ShortRead::FastqStreamer(file)
    fq <- ShortRead::yield(strm)
    close(strm)
    return(fq)
  }

  expect_identical(
    readfq("https://raw.githubusercontent.com/mritchielab/FLAMES/devel/tests/testthat/demultiplexed.fq"),
    readfq(file.path(outdir, "out.fq"))
  )
})
