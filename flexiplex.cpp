...
vector<Barcode> big_barcode_search(string & sequence, unordered_set<string> & known_barcodes,
  ...
  //search for barcode
  Barcode result=get_barcode(sequence,&known_barcodes,max_flank_editd,max_editd);
  ...
  if(return_vec.size()>0){
    ...
    masked_res=big_barcode_search(masked_sequence,known_barcodes,max_flank_editd,max_editd);
